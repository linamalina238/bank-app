...
Цей проєкт розроблено як архітектурний прототип (MVP). Ми свідомо пішли на певні спрощення для пришвидшення розробки, проте ми чітко розуміємо вектори розвитку системи для перетворення її на стабільний Production-ready продукт.

### 1. Архітектура та Бази Даних

- **Перехід на SQL (PostgreSQL/SQLite):** Заміна `data.json` на реляційну БД для забезпечення **ACID-транзакційності**. Це критично для фінансових систем, оскільки гарантує, що дані не будуть втрачені або пошкоджені при одночасних запитах (**Race Condition**).
- **Asynchronous I/O:** Повна заміна синхронних методів `fs.readFileSync` на асинхронні `fs.promises`. Це дозволить розблокувати **Event Loop** сервера та забезпечити високу пропускну здатність (Concurrency).

### 2. Безпека (Security)

- **Refresh Tokens:** Впровадження механізму оновлення сесій через Refresh Tokens, що зберігаються у `httpOnly` куках. Це мінімізує ризики XSS-атак.
- **Server-side Validation:** Інтеграція бібліотек валідації схем (наприклад, **Zod** або **Joi**). Ми дотримуємося принципу _"Ніколи не довіряй клієнту"_, тому кожен байт вхідних даних має проходити жорстку перевірку на бекенді.

### 3. Технологічний Стек

- **TypeScript:** Міграція всього проєкту на TS для впровадження статичної типізації. Це дозволить виявляти помилки типу `undefined` або невідповідності інтерфейсів ще на етапі розробки.
- **React:** Перехід на компонентну архітектуру та використання State Management (Zustand/Redux) для масштабування користувацького інтерфейсу та покращення UX.

### 4. Тестування та Надійність

- **Unit & Integration Testing:** Покриття критичної бізнес-логіки (розрахунок балансу, валідація JWT, перекази) автоматичними тестами за допомогою **Jest**.
- **CI/CD Pipeline:** Налаштування процесів автоматичного тестування та деплою при кожному Pull Request, щоб гарантувати стабільність головної гілки репозиторію.
